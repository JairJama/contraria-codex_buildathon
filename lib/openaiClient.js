...
const WEB_EVIDENCE_MODEL = 'gpt-5.6-luna';
const WEB_EVIDENCE_TIMEOUT_MS = 9_000;
const WEB_EVIDENCE_MAX_SOURCES = 2;
...
function formatWebEvidence(webEvidence) {
  if (!webEvidence?.summary || !webEvidence.citations?.length) {
    return 'No se consultó evidencia web adicional. Distingue con claridad los hechos del contexto de los supuestos.';
  }

  const sources = webEvidence.citations
    .map((citation) => `[${citation.id}] ${citation.title}: ${citation.url}`)
    .join('\n');

  return `Resumen de investigación externa (puede estar incompleto):\n${webEvidence.summary}\n\nFuentes verificables disponibles:\n${sources}\n\nÚsala solo como contexto complementario: no inventes datos, URLs ni citas fuera de estas fuentes.`;
}

export async function evaluateAgent({ agent, round, idea, context, priorPositions = [], webEvidence, deadlineAt }) {
  return requestWithBudget({
    deadlineAt,
  ...
  for (const item of response.output || []) {
    ...
    for (const content of item.content || []) {
      for (const annotation of content.annotations || []) {
        ...
        if (annotation?.type !== 'url_citation' || typeof url !== 'string' || seenUrls.has(url)) continue;
        ...
        seenUrls.add(url);
        citations.push(WebCitationSchema.parse({
          id: `S${citations.length + 1}`,
          title: String(source.title || new URL(url).hostname).trim().slice(0, 500),
          url,
        }));

        if (citations.length === WEB_EVIDENCE_MAX_SOURCES) return citations;
      }
    }
  }
...
export function createEmptyWebEvidence({ requested = false, status = 'not_requested' } = {}) {
  return {
    requested,
    status,
    summary: '',
...
export async function researchWebEvidence({ idea, context, deadlineAt }) {
  const researchDeadlineAt = Math.min(deadlineAt, Date.now() + WEB_EVIDENCE_TIMEOUT_MS);

  return requestWithBudget({
    deadlineAt: researchDeadlineAt,
    fallback: () => createEmptyWebEvidence({ requested: true, status: 'fallback' }),
    logContext: { provider: 'openai', operation: 'web_evidence' },
    request: async (signal) => {
      const response = await getClient().responses.create({
        model: WEB_EVIDENCE_MODEL,
        input: [
          {
            role: 'system',
            content: 'Eres un investigador breve para un debate de ideas. Busca en la web solo evidencia externa directamente útil para evaluar la propuesta. Prioriza fuentes primarias, institucionales o sectoriales fiables. Devuelve un resumen factual en español de máximo 900 caracteres, con solo afirmaciones respaldadas por las fuentes encontradas. No des un veredicto ni inventes URLs.',
          },
          {
            role: 'user',
            content: `Idea a evaluar:\n${idea}\n\nContexto adicional:\n${context || 'No proporcionado'}\n\nBusca evidencia reciente y relevante. Usa como máximo dos fuentes en tu respuesta.`,
          },
        ],
      ...
      return {
        requested: true,
        status: 'complete',
        summary: String(response.output_text || '').trim().slice(0, 900),
...
export async function createFinalVerdict({ idea, context, round1, round2, webEvidence, deadlineAt, remainingMs: globalRemainingMs }) {
  return requestWithBudget({
    deadlineAt,
    fallback: (reason) => ({
